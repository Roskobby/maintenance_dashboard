...
system_type_options = ['All'] + list(df['SystemTypeValue'].dropna().unique())
...
failure_type_options = ['All'] + list(df['FailureTypeValue'].dropna().unique())
...
work_status_options = ['All'] + list(df['WorkStatusValue'].dropna().unique())
...
if 'All' not in selected_system_types:
    filtered_df = filtered_df[filtered_df['SystemTypeValue'].isin(selected_system_types)]
if 'All' not in selected_failure_types:
    filtered_df = filtered_df[filtered_df['FailureTypeValue'].isin(selected_failure_types)]
...
with elements("dashboard"):  
    mui.Grid(container=True, spacing=2, children=[
        ...
        mui.Paper(children=[mui.Typography("Open Work Orders"), mui.Typography(f"{len(filtered_df[filtered_df['WorkStatusValue'].isin(open_work_statuses)])}", variant="h4")], sx={"padding": 2, "backgroundColor": "#FFEBEE"}),
        mui.Paper(children=[mui.Typography("Completed Work Orders"), mui.Typography(f"{len(filtered_df[filtered_df['WorkStatusValue'] == 'Completed'])}", variant="h4")], sx={"padding": 2, "backgroundColor": "#FFF3E0"}),
        mui.Paper(children=[mui.Typography("Closed Work Orders"), mui.Typography(f"{len(filtered_df[filtered_df['WorkStatusValue'] == 'Closed'])}", variant="h4")], sx={"padding": 2, "backgroundColor": "#ECEFF1"})
    ])

# Work Orders by Location
if not filtered_df.empty:
    location_chart = px.bar(filtered_df['ParentLocationValue'].value_counts().reset_index(), x='ParentLocationValue', y='count', title='Work Orders by Location', labels={'ParentLocationValue': 'Location', 'count': 'Count'})
    st.plotly_chart(location_chart)

# Work Orders by Priority Level
if not filtered_df.empty:
    priority_chart = px.bar(filtered_df['WorkPriorityValue'].value_counts().reset_index(), x='WorkPriorityValue', y='count', title='Work Orders by Priority', labels={'WorkPriorityValue': 'Priority Level', 'count': 'Count'})
