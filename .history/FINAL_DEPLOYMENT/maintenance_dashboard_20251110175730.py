...
# Put page config as the first Streamlit call
st.set_page_config(page_title="GPMS Maintenance Dashboard", page_icon=":bar_chart:", layout="wide")

# EXE-safe base path helper (works in VS Code and after PyInstaller)
BASE_DIR = getattr(sys, "_MEIPASS", os.path.dirname(os.path.abspath(__file__)))
def local_path(name: str) -> str:
    return os.path.join(BASE_DIR, name)

# ✅ Use EXE-safe paths for your data files
kpi_path = local_path("kpi_manual.html")

# Compact download link for KPI manual
if os.path.exists(kpi_path):
    with open(kpi_path, "rb") as f:
        b64 = base64.b64encode(f.read()).decode()
    st.markdown(
        f'<p style="font-size:16px;">📖 <a href="data:application/octet-stream;base64,{b64}" '
        f'download="kpi_manual.html">Download KPI Manual (HTML)</a> to view metric definitions.</p>',
        unsafe_allow_html=True
    )
else:
    st.markdown(
        '<p style="font-size:16px;color:red;">Error: KPI Manual not found.</p>',
        unsafe_allow_html=True
    )
...
def load_data_uncached():
    ...
    """
    file_path = local_path("Asset Work History.xlsx")  # Use executable-safe path  
    try:
    ...
    with st.expander("🔧 Buoy Bush Change-Out Reliability"):
        ...
        if not filtered_df.empty:
            reliability_query = """
                ...
                AND ("WorkDescription" ILIKE '%UKP%Bush%Change out%' 
                     OR "WorkDescription" ILIKE '%Change out%UKP%Bush%'
                     OR "WorkDescription" ILIKE '%Replaced%UKP%Bush%'
                     OR "WorkDescription" ILIKE '%UKP%Bush%Replaced%')
