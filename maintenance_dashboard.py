...
with st.expander("📈 Maintenance Performance KPIs"):
    ...
    col2.metric(label="Mean Time to Repair (MTTR)", value="N/A")  # Requires duration calculation
    col3.metric(label="Mean Time Between Failures (MTBF)", value="N/A")  # Requires failure interval data
...
if not filtered_df.empty:
    ...
    location_chart = px.bar(
        location_counts,
        x='ParentLocation', y='count',
        title='Work Orders by Location',
        labels={'ParentLocation': 'Location', 'count': 'Count'}
    )
...
if not filtered_df.empty:
    ...
    priority_chart = px.bar(
        priority_counts,
        x='WorkPriority', y='count',
        title='Work Orders by Priority',
        labels={'WorkPriority': 'Priority Level', 'count': 'Count'}
    )
